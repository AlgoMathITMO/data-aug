...
def data_processing(df_movie: pd.DataFrame, 
    ...
    df_rating_clean = df_rating[['userId', 'movieId', 'rating']]
    ...
    df_rating_clean = df_rating_clean.rename(columns={'movieId': 'item_idx', 'userId': 'user_idx'})
...
def title_prep(title: str) -> str:
    ...
    """
    Clear text with regular expressions.
...
def extract_year(title: str) -> Optional[str]:
    ...
    """
    Extraction year from movie title.
...
def genres_processing(movies: pd.DataFrame) -> pd.DataFrame:   
    ...
    """
    Processing of genres, encode genres to multidimensional vector. 
    
    Arguments:
    --movies: genres list.
    
    Return:
    --movies_return: genres vector.
...
def fill_null_years(movies: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    Processing of null years. 
    
    Arguments:
    --movies: initial dataframe.
    
    Return:
    --df_movies: dataframe with no null years.
